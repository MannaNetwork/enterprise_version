...
$demo_coin_array = $priceslotinfo->getBuyersBalanceTN($user_id);
//returns array $balance_btc, $tn_balance)
$demo_coin = $demo_coin_array[1];
$demo_coin = round($demo_coin, 8, PHP_ROUND_HALF_UP);
?>
...
$priceslotinfo->credit_testnet ($_POST["buyer_user_id"], $_POST["transfer_amount"]);
$priceslotinfo->debit_testnet ($user_id, $_POST["transfer_amount"]);
...
echo '</h2>';
echo '<h2> The recipient\'s user id is ', $buyer_user_id; 
echo '</h2>';
$msg="
...
<h2>The Process</h2>
<h3>Simply enter the amount of demo coins you wish to transfer and the user ID of the user you wish to transfer them to</h3>
<h3>You have ";
$msg .=  $demo_coin;
$msg .= "<b><u> DEMO COIN</u></b> available to transfer. (Note either you or the user may request more free demo coin from the admin via the contact form)</h3>
<form name=\"test\" action=\"". htmlentities($_SERVER['PHP_SELF'])."\" method=\"post\">
<tr><td><h3>Enter the USER ID of the BungeeBones User you wish to transfer the demo coins to - </h3>
<input type='text' name='buyer_user_id'  size='5'> </td></tr>
<tr><td><h3>Enter the amount of DEMO coin (non-redeemable for Bitcoin) you want to transfer</h3>
