...
$file="http://exchange.manna-network.com/incoming/install_advertiser_agent_bridge.php";
...
$file="http://exchange.manna-network.com/incoming/install_advertiser_agent_bridge.php";
		...
		curl_setopt($ch, CURLOPT_URL, $file);
		//curl_setopt($ch, CURLOPT_POSTFIELDS,$args);
		curl_setopt($ch, CURLOPT_POSTFIELDS,'agent_id='.AGENT_ID.'&remote_user_id='.$user_id.'&remote_link_id='.$link_id.'&recruiter_lnk_num='.$recruiter_lnk_num.'&user_registration_datetime='.$user_registration_datetime.'&website_title='.utf8_encode(stripslashes(trim($website_title))).'&website_description='.utf8_encode(stripslashes(trim($website_description))).'&website_url='.utf8_encode($website_url).'&category_id='.$category_id.'&selected_cat_id='.$selected_cat_id.'&selected_region_id='.$selected_region_id.'&newcatsuggestion='.utf8_encode(stripslashes(trim($newcatsuggestion))).'&location_id='.$location_id.'&website_street='.utf8_encode(stripslashes(trim($website_street))).'&map_link='.utf8_encode($map_link).'&website_district='.utf8_encode(stripslashes(trim($website_district))).'&installer_id='.$installer_id.'&promo_credit='.$promo_amount);
