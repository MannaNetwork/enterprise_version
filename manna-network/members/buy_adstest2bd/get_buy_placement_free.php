...
echo '<p>Advertisers in BungeeBones are encouraged to participate in the bidding program. You can do so by funding your account with either Testcoin (which is absolutely free) or with real Bitcoin (which you can acquire through a number of means). 
<p>Paying advertisers\' links are displayed ahead of non-paying links so this credit will enable you to move your listing ahead of all the non-paying links.
 <h3>Add A Free BungeeBones Web Directory Script And Earn Some Bitcoin Or Testcoin Yourself!</h3>
<p>To start using your credit just scroll down through the row of potential price slots below until you find the radio button of the amount you want to spend. Then click the "Process Now" button to submit the form.
...
$price_slot_range = $LINKinfo->getDisplayConfigPrice($user_id, $selected_link_id,  $coin_type); 
  $satoshi_display = $LINKinfo->getDisplayConfigPrefix($user_id, $selected_link_id,  $coin_type); 
  ...
  $link_display = $LINKinfo->getUserByLinkId($selected_link_id);
	   ...
	   $db_category =$link_display[2]; 
			...
			$cat_name= $catPop_arrp[2];  
					$num_pages_array = $price_slot_info->howManyRadiosPaid($db_category, $db_id, "free");
...
$display_block .= "<br><b> in Bitcoin based advertising credits available";
}
if($balance_testcoin > 0){
$display_block .= "<br><b><p> You have ";
$display_block .=  $balance_testcoin;
$display_block .= "<br><b> in Testcoin based advertising credits available";
}
...
if($t <= $high AND $t >= $low){ 
					...
					$num_links_in_slot = $price_slot_info->getNumLinksInSlot($db_category, $incr_slot_price, "free");
