<? 
echo '<h1>TODO - As soon as we can, discover the current listings cointype - if it is the opposite of this modify type (bitcoin) then update the coin type from testcoin. Also, do not display the revious price slot info</h1>';
...
if($previous_coin_type == "testcoin"){
...
if($satoshi!=1  AND $switchtype == "to_bitcoin"){
...
$incr_slot_price = rtrim(rtrim($incr_slot_price, "0"),".");
				...
				$num_links_in_slot = $price_slot_info->getNumLinksInSlot($db_categoryt, $incr_slot_price, "testcoin");
									...
									if($incr_slot_price == $user_purchase_info[2]  AND $switchtype == "to_bitcoin"){
							...
							$display_blocktn1 .= "<td STYLE='font-size: 100%;'>No Links</td></tr>";
									...
									elseif($incr_slot_price == $user_purchase_info[2]  AND $switchtype == "to_bitcoin"){
