...
if($previous_coin_type == "bitcoin"){

$switchtype = "to_testcoin";
echo '<h2>Move Or Cancel Your Link Position</h2>';
echo '<p>To cancel your paid listing just scroll down to the row of your current price slot (it will be highlighted in green) and select it. 
...
{
echo '<h2>You are about to cancel your Testcoin Price Slot and select a Bitcoin Price Slot (That could be a big gain in position). If that is not what you intended close this modal window (top right).</h2>';
$switchtype = "to_bitcoin";
}
...
echo '<br>line 150 get modify placement bitcoin switch = ', $switchtype;
if($satoshi!=1 AND $switchtype == "to_testcoin"){
...
if($incr_slot_price == $user_purchase_info[2] AND $switchtype == "to_testcoin"){
							...
							$display_block1 .= "<td STYLE='font-size: 100%;'>No Links</td></tr>";
									...
									elseif($incr_slot_price == $user_purchase_info[2]  AND $switchtype == "to_testcoin"){
