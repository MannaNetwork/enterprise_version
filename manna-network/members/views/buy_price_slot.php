<?php
$display_block = file_get_contents(dirname( __FILE__, 2 ).'/css/members_menu.css');
...
require_once(dirname( __FILE__, 5 )."/".AGENT_FOLDERNAME."/manna-network/members/classes/member_page_class.php");//load order 1
...
$this_links_status_on_Central = $_POST['this_links_status_on_Central'];

$reason="downgrade";
$this_links_status_on_Central = $_POST['this_links_status_on_Central'];
$linkInfo = new member_page_info();
//Change on local table
//$linkInfo->recordModifyPriceslotsSubscripts($previous_id,$user_id, $prev_link_id,$prev_agent_ID,$prev_start_date,$prev_amount,$new_price,$reason);
$linkInfo->updateLocalPriceslotsSubscripts($user_id, $agent_ID, $link_id, $new_price, $old_price, $cat_id, $installer_id, $coin_type);
$linkInfo->sendModifyToCentral($user_id, $agent_ID, $link_id, $new_price, $old_price, $cat_id, $installer_id, $coin_type, $reason, $this_links_status_on_Central);
get_footer();
exit();
...
<input type='hidden' name='cat_sta/home/robert/Documents/workbench/temp/tus' value='".$_POST['cat_status']."'>
<input type='hidden' name='new_bid_type' value='".$_POST['new_bid_type']."'>
<input type='hidden' name='this_links_status_on_Central' value='".$_POST['this_links_status_on_Central']."'>

<input type='hidden' name='confirm_downgrade' value='true'>
";

$steps_display .= '<h1>Confirm Your Downgrade!</h1><h2> Your listing will receive a new Bid Seniority. Your new position in the display may be affected. Ads are displayed first by price slot amount and then by their seniority (i.e. date/time purchased) withing that price slot.  </h2>
<br><input type="submit" name="confirm_downgrade" id="confirm_downgrade" value="Confirm downgrade" /> </td></tr></table>';
...
<input type='hidden' name='coin_type' value='".$_POST['coin_type']."'>
<input type='hidden' name='agent_ID' value='".$_POST['agent_ID']."'>
<input type='hidden' name='cat_status' value='".$_POST['cat_status']."'>
<input type='hidden' name='new_bid_type' value='".$_POST['new_bid_type']."'>
<input type='hidden' name='this_links_status_on_Central' value='".$_POST['this_links_status_on_Central']."'>
...
$linkInfo = new member_page_info();
$linkInfo->sendModifyToCentral ($current_id,$user_id,$link_id, $agent_ID,$prev_start_date,$prev_amount,$new_price, $coin_type, $reason, $this_links_status_on_Central);
get_footer();
exit();
}
...
echo '<br>in buy price slot php line 258 - link_id = ', $link_id;
echo '<br>in buy price slot php line 258 - location_id = ', $location_id;
...
$site_is_reviewed = $linkInfo->isUserLinkReviewed($user_id, $agent_ID, $link_id, $cat_id);
...
//If it returns a string with a pipe in it then the high bid will be on the left and the low one on the right (note they will be the same if all the bids are the same)
 $this_links_status_on_Central = $linkInfo->getUserPriceSlots($user_id, $agent_ID, $link_id, $cat_id, $coin_type);
// returns either "no_bids", "temp_bid", "approved_bid", or "error_detecting_bid"
	if($debug=="2"){
	echo '<h3>$this_links_status_on_Central = '.$this_links_status_on_Central. '</h3>';
	}
if($this_links_status_on_Central !=="no_bids"){
	if($debug=="2"){
...
//https://1stbitcoinbank.com/manna_network/manna-network/members/buy_price_slot.php?link_id=79&price=&cat_id=60&coin_type=DMC&agent_ID=17&users_balances_string=0000000000.0000000000|0000000000.1000000000&this_links_status_on_Central=temp_bid&this_links_bid=000.00127836&tregional_num=y&B1=%271%27
...
$this_links_status_on_Central= $_GET['this_links_status_on_Central'];
$this_links_bid= $_GET['this_links_bid'];
$tregional_num = $_GET['tregional_num'];
}
echo file_get_contents('views/css/style.css');
...
// returns either "no_bids", "temp_bid", "approved_bid", or "error_detecting_bid"
	...
	$isLinkApproved = $linkInfo->isUserLinkReviewed($user_id, $agent_ID, $link_id, $cat_id);
	echo '<br>$isLinkApproved = ', $isLinkApproved;
	/*
...
include('views/_menu.php');
	...
	//this cat status may still have this user in temp status
//echo '<br>line 324 first if and $this_links_bid_status_on_Central = ', $this_links_bid_status_on_Central;
	if($debug=="2"){
echo '<br> line 362 in elseif($cat_status_approved == 0) section';
...
$hi_low_string = $linkInfo->getMinMaxPriceSlotFromCentral($cat_id, $coin_type);
	...
	//this user status may be the one causing the cat to "have bid" so we need to 1) check if this user has an approved bid , then this user is only bid
		...
		$this_links_rank = 1;  //has to be, is the only one
...
{
if(null ==($_GET['link_id']) AND null ==($_POST['link_id'])){
//header("Refresh:0; url=index.php");
}
require_once(dirname( __FILE__, 4 )."/manna-configs/db_cfg/agent_config.php");
 include($_SERVER['DOCUMENT_ROOT']."/".AGENT_FOLDERNAME."/manna-configs/db_cfg/agent_config.php");
include('styles.css');
get_header();
echo file_get_contents(dirname( __FILE__, 2 ).'/css/members_menu.css');
include('views/_menu.php');
