...
$display_blockmp.= '<h3 align="center" style="padding:5px 10px;">'.EDIT_FORM_WELCOME_TITLE.'</h3>';
echo'
...
$display_blockmp .= '<input type="hidden" name="remote_link_id" value="'.$_GET['link_id'].'">';
...
$affiliate_link_display = $LINKinfo->getLinkByLinkId($_GET['link_id']);
//getRegionsSubTree($location_id)
...
$protocol= $affiliate_link_display[6];
$website_url= $affiliate_link_display[7];
$category_id= $affiliate_link_display[8];
$location_id= $affiliate_link_display[9];
$website_street= $affiliate_link_display[10];
$page_name= $affiliate_link_display[11]; 
$map_link = $affiliate_link_display[12]; 
$bridge_id= $affiliate_link_display[13];
$user_registration_datetime= $affiliate_link_display[14];
$installer_id= $affiliate_link_display[15];

$display_blockmp .= '  <label for="website_title">'. WORDING_EDIT_TITLE.'</label><span class="dropt" style="font-size: large;" title="'.REG_BLOKT_TITLE_MOUSEOVER.'"><img height="21" width="21" src="images/info_icon.png">
  <span style="width:500px;">'.REG_BLOKT_TITLE_MESSAGE.'</span></span>
   <br>   <input id="website_title" type="text"  name="website_title" style="width: 20em;" value="'.$website_title.'" required />
    <br>   <label for="website_description">'. WORDING_EDIT_DESCRIPTION.'</label><span class="dropt" style="font-size: large;" title="'.REG_BLOKT_DESCRIPTION_MOUSEOVER.'"><img height="21" width="21" src="images/info_icon.png">
  <span style="width:500px;">'.REG_BLOKT_DESCRIPTION_MESSAGE.'</span></span>
     <br>   <textarea  id="website_description" type="text-area"  name="website_description" value="'.$website_description.'" required  rows="5" cols="40">'.$website_description.'</textarea>
  ...
  if($protocol=="https://"){
     $display_blockmp .= '<input type="radio" name="protocol" id="protocol" value="https://" checked="checked" />
 ...
 $display_blockmp .= '<input type="radio" name="protocol" id="protocol" value="https://"  />
            <label for="protocol">https://</label> <br>
            <input type="radio" name="protocol" id="protocol" value="http://" checked="checked" />
            <label for="protocol">http://</label>';
            }
        
 $display_blockmp .= '<br>   <label for="website_url">'. WORDING_REGISTRATION_URL.'</label><span class="dropt" style="font-size: large;" title="'.REG_BLOKT_URL_MOUSEOVER.'"><img height="21" width="21" src="images/info_icon.png">  <span style="width:500px;">'.REG_BLOKT_URL_MESSAGE.'</span></span>
      <br>    <input size="50" id="website_url" type="text"  name="website_url"  value="'.$website_url.'" required />
      </br>
...
$categoriesTree = $LINKinfo->get_categories_array($category_id);
$countCatLevels = count($categoriesTree[1]) - 1;
$crumbtrailC = "";
for($ccl=1;$ccl<=$countCatLevels;$ccl++){
$crumbtrailC .= '->'.$categoriesTree[1][$ccl];
} 
	//WORDING_EDIT_CATEGORY	
$display_blockmp .= '<span id="mn_subcat_container"> 
 <table id="mn_subcat_table"><tr><td colspan="2">'.$crumbtrailC.CAT_TREE_EXPLANATION.'</td></tr>
<tr><td><h2>'.EDIT_CATEGORY_HEADING.'</h2></td></tr><tr><td>';

$display_blockmp .= '<select name="submenu" onchange="showSubMenu(this.value,\'1\',\'' .  $selected_region_id  . '\',\'categories\')"><option value="">' .  WORDING_AJAX_MENU1  . '</option> ';
$categoriesRow = $LINKinfo->getCategoriesRow($category_id);

$categoriesName = $LINKinfo->getCategoriesName($category_id);
$categoriesLftRgt = $LINKinfo->getCategoriesLftRgt($category_id);
$catsByParent = $LINKinfo->getChildrenByParent(1);
...
<input style="font-size: larger; font-weight:stronger;width: 250px;" type="text" id="selected_cat_menu_name"  class="selected_cat_menu_name" name="selected_cat_menu_name" value=""><br>   <label for="catkeys">'. WORDING_CATKEY_DESCRIPTION.'</label><span class="dropt" style="font-size: large;" title="'.REG_BLOKT_CATKEY_MOUSEOVER.'"><img height="21" width="21" src="images/info_icon.png">
  <span style="width:500px;">'.REG_BLOKT_CATKEY_MESSAGE.'</span></span>
   <br>  <textarea  id="catkeys" type="text-area"  name="catkeys" rows="5" cols="40"></textarea></td><td></table>';
	//}
	$display_blockmp .= '<span id="mn_location_container"> 
<table id="mn_location_table">';
if($location_id > 0){
$locationTree = $LINKinfo->getRegionsSubTree($location_id);
$countLocLevels = count($locationTree[2]) - 1;
$crumbtrailR = "";
for($lt=1;$lt<=$countLocLevels;$lt++){
$crumbtrailR .= '->'.$locationTree[2][$lt];
} 
$display_blockmp .= '<tr><td colspan="2">'.$crumbtrailR.REG_TREE_EXPLANATION.'</td></tr>';
}
$display_blockmp .= '<tr><td><h2>'.EDIT_CATEGORY_HEADING.'</h2></td></tr><tr><td><select name="regional" id="regional" onchange="
