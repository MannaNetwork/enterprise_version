...
$price_credits_4sale = $_POST['price_cre{dits_4sale']; 
...
<ul><li>To authorize Manna-Network.com to release said account number and/or name to a perspective buyer when they commit to buying your advertising credits</li>
<li>To hold Manna-Network harmless for any results or damages you suffer by doing so.</li>
</ul>";
}
else
{
...
$msg .=   "<h3  style = 'text-align:center;'>Overview Of The Manna-Network Peer To Peer Advertising Credit/Pseudo Bitcoin Exchange</h3>
<table id='member'><tr><td><p style=\"font-size:120%; text-align:left;\" >You are about to offer all or part of your advertising credits For Sale to other Manna-Network members. Remember, each advertising credit is redeemable for 1 BitcoinSV from Manna-Network. Each section below lets you set the terms that must be met for the transaction to complete. Afterward, you will be presented a confirmation page and then those advertising credits will become \"frozen\" and you will not be able to use them for your own advertising nor be able to redeem them for BitcoinSV. If, however, you decide to remove them from the market, you can do so at any time <b>before anyone has accepted your offer</b>. Then they will be \"unfrozen\" and available again to you.
...
echo '<br>'. dirname(__DIR__, 2)."/manna-network/";
include(dirname(__DIR__, 2)."/views/modal/btc_price/tickers/ticker_usd_btc_bitstamp.php");
$handle = curl_init();
...
//$monthly_minimum_bid_target = $monthly_target_fee_in_dollar_value/$data['USD'];
if(!isset($price_credits_4sale  )){
$price_credits_4sale = $data['USD'];
$msg .=   "(The Default Price Entered Is The Current BitcoinSV Price. The price offered to the perspective buyers will be the then current BitcoinSV price at that future time. )";
}
else
{
$msg .=   "(The Current BitcoinSV Price Is".$data['USD'].")";
}
