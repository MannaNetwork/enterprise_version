...
if($row_cnt == 0){
echo '<h1>You must have registered in order to download the php script</h1>';
exit();
}
elseif($row_cnt > 0){
...
// while ($row = mysqli_fetch_array($result)) 
  ...
  //               		$link_id[] = $row['id'];
//		          }
echo '<h1>Line 56 of phpblock_download.php - user has multiple sites and need a selection form or error handler to identify which to associate with this download and which affiliate number to give them for configuration.</h1>';
print_r($BB_user_id);
exit();
}
...
$moniker="<h5>Congratulations On Getting Your Plugin To Start Earning Bitcoin!</h5>";
$body_width="wide";
...
<h1 style="font-size:350%;">Download BungeeBones Blog4Bitcoin PHP Script</h1>


<a href="/phpblock/bungeebones_php_version.zip"><div  class="grid_12"  style="background-color:yellow; width:100%; height:520px; border: 2px solid; border-radius: 25px;">
<h1>Click To Download Your Script</h1>
<ul style="font-size:154%;">
<li>It Is Free!</li>
<li>It Will Earn Bitcoin For Ad Sales It Generates</li>
...
<h1>Download And Install Your Script Now!</h1>
</div> </a>
...
include("../960bottom.php");
    ...
    include("views/phpblock_not_logged_in.php");
}
