...
$cat_group_pops = $this->getCatGroupPops($cat_id, $val, "con");
$cat_group_pops_string = implode(",", $cat_group_pops);
$cat_sub_group_pops = $this->getCatSubGroupPops($cat_id, $val, "con");
...
$query = 'SELECT * FROM `categories` WHERE `parent`= "'.mysql_escape_string($cat_id).'  "ORDER BY `name` ASC   ';
$result = mysql_query($query, $connect);
	while($row = mysql_fetch_array($result)){
	if($regional_num >1){ 
