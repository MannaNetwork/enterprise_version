...
if(array_key_exists('referer_lnk_num', $_POST) AND $_POST['referer_lnk_num']  =="change_me")
                {   
...
echo '<h1 style="color:red";>RETURN TO PREVIOUS PAGE, <a href="http://'.$_POST['remote_server'].'/mannanetwork-dir/index.php">PREVIOUS PAGE</a> SELECT "ADD URL" TAB AGAIN, AND THE CORRECT CONFIGURATION SHOULD CAUSE THIS NOTICE TO NOT APPEAR</h1>';
exit();
}
else
{
$wp_host_url = $_POST['remote_server'];
...
// but this is only run inside the function
		...
		<p>&nbsp;<p><span  style='font-weight:bold;'><h3><a href='https://".AGENT_URL."/".AGENT_FOLDERNAME."/manna-network/members/register.php?referer_lnk_num=0&remote_server=".$_SERVER['HTTP_HOST']."&flag=1'>https://".$_SERVER['HTTP_HOST']."/".AGENT_FOLDERNAME."/manna-network/members/register.php?referer_lnk_num=0&remote_server=".$_GET['remote_server']."&flag=1</a>";
			...
			echo '<h1>You are about to make a ONE-TIME configuration of what will be your adminstrative account, thus making this domain (i.e. '. $cleaned_host. ') the "enterprise/agency" website. Be sure the information is accurate and there are no "typos"</h1>';
