<?php
$submit_button_name = "purchase";
$button = "Lower Price"; 
$link_title = "Downgrade To Less Expensive Price Slot";
$pre_blockt_text_message  = "Downgrade Your Bid!";
$mouseover = "You can Downgrade your price slot"; 
$blockt_message = "Any price slots without previous buyers (like this one) would mean yours still would be displayed ahead of all lower price slots and free listings and ahead of all future buyers of this same priceslot in every directory across the network. Since there are no previous buyers in this price slot then yours would have seniority over any later buyers in it. By downgrading, however, it will be easier(i.e. less expensive) for others to take the top position. Strategy mostly involves your forecasts of the BitcoinSv price. If prices rise, other higher price slots may choose to drop down to lower price slots (which would raise your position in the directory). if prices drop, other advertisers are more likely to buy higher ones than yours. See <a target = '_blank' href='translations/en/bidding_tips.php'>Bidding Tips</a> for more info on strategy.<span style='color:red;'>Disclaimer: Any statements regarding your position are subject to possible other purchases in this same slot made earlier than yours and/or higher priceslots bought today.</span>"; 

?>
