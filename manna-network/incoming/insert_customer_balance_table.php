<?php echo 'in update customer balance tables';
print_r($_POST);
...
echo '<br> $_POST[amount_DMC_hash_key] ; = ', $_POST['amount_DMC_hash_key'] ;
if($_POST['amount_DMC_hash_key'] == $amount_DMC_hash_key){

//insert the POST data into balance table

$sql = "SELECT * FROM balance WHERE `user_id` =$remote_user_id";
echo '<br>', $sql;
$result = mysqli_query($mysqli, $sql) or die("Couldn't execute 'Edit 16 Account' query");
while($row = mysqli_fetch_array($result)){
...
$sql = "UPDATE balance SET 'amount_DMC' = ".$NEW_amount_DMC."WHERE `user_id` =".$remote_user_id;
echo '<br>', $sql;
$result = mysqli_query($mysqli, $sql) or die("Couldn't execute 'Edit 16 Account' query");
